# Custom greetings
exports.greetings =
  "4e18f194a3f75133bd07148a": "Who's ready to get dirty?!" # DirtyGirlfriend
  "4e2202d8a3f751739902ecea": (user) -> "/me awkwardly hugs #{user.name}" # .Mnml_Pixels
  "4e48b4efa3f75104420c8aa1": (user) -> "/me daps #{user.name}"
  "4e0543b4a3f7517602073376": "It's everybody's favorite cinnabon!"
  "DJ Wooooo": "This party must really be poppin', DJ Wooooo is in the house!"
  "4e1a6db9a3f75162f5018c05": (user) -> "Watch out, #{user.name} is on the prowl!" # Elletiger
  "4e2a589c4fe7d0158608ac54": (user) -> "I await your command, master #{user.name}"
  "Frick": "SCATTER! It's bot overlord Frick!"
  "4e0b90f6a3f75146750a09cb": (user) -> "My name is #{user.name}. We are Cybernetic Dubstep Organisms from the planet Wubwubwub." # Dima/Darkstar
  "4dfa5e724fe7d056c20c9db6": (user) -> "Everyone greet #{user.name}, the prettiest girl on this PARTY BUS!" # GalGal
  "IT'S OVER 9000!!!!": "Vegeta, what does the scouter say about his power level? It's OVER 9000!!!!"
  "Jellytime": "It's peanut butter JELLYTIME!"
  "4de56d41e8a6c47401000d74": (user) -> "Hey, I think #{user.name} is AFK!" # Nick the K
  "4df672394fe7d04a1901cbeb": (user) -> "#{user.name} is here and he brought Chipotle!"
  "4e827db5a3f7511650115ee1": "Konnichiwa, Stacy-san! :)"
  "4e19470fa3f75133a90951cc": (user) -> "And I said to #{user.name}: La gente esta muy loca! What the f*ck!?"
  "sex": "Who wants sex?"
  "vuther": "Hey, papa vuther is here on the PARTY BUS!"
